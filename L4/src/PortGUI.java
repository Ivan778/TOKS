            ...
            try {
                ...
                for (int i = 0; i < message.length(); i++) {
                    ...
                    while (attempt < 10) {
                        ...
                        serialPort.getSerialPort().writeString(byTE);
                        long currentTime = Instant.now().toEpochMilli() / 1000L;

                        if (currentTime % 2 == 0) {
                            System.out.println("Collision");

                            serialPort.getSerialPort().writeString(jam);

                            attempt++;
                            int end = (int) Math.pow(2, attempt);

                            Random rand = new Random();
                            int delay = rand.nextInt((end));
